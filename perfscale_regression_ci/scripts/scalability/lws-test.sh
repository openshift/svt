...
function wait_for_running_pod {
  ...
  if [ "$RETRY" -gt $max_retry ]; then
      echo "Timed out waiting for all pods in namespace '$NAMESPACE' to be Running."
  else
  ...
  TOTAL_RUNNING_PODS=$(oc get pods -n "$NAMESPACE" --no-headers | grep Running | wc -l)
...
echo "Running in project $NAMESPACE"
echo "====================="
oc delete project "$NAMESPACE" || true
oc new-project "$NAMESPACE" > /dev/null 2>&1
if [[ $? -ne 0 ]]; then
    echo "Failed to create project $NAMESPACE!"
    exit 1
fi
...
if [[ $DELETE ]]; then
    ...
    oc get po -n lws-test --no-headers | awk '{print $1}' | head -n 1 | xargs oc -n "$NAMESPACE" delete pod
    wait_for_running_pod
fi