...
## moves pods using local storage to new nodes
## Be sure to follow steps in README.md to install descheduler and correct profiles for this test
## Expected profiles:
##    - TopologyAndDuplicates
##    - EvictPodsWithLocalStorage
## Polarion test case: OCP-44240
## https://polarion.engineering.redhat.com/polarion/#/project/OSE/workitem?id=OCP-44240
## Cluster config: 3 master (m5.xlarge or equivalent) with 3 workers
## kube-burner config: perfscale_regerssion_ci/kubeburner-object-templates/descheduler-local-storage.yml
## optional PARAMETERS: number of JOB_ITERATION
################################################ 

source ../../common.sh
source common_func.sh
source local_storage_and_pvc_env.sh
...
if [[ ${pass_or_fail} == 2 ]]; then
  echo -e "\nOverall Descheduler - Validate EvictPodsWithLocalStorage & default for EvictPodsWithPVC profiles Testcase result:  PASS"
  ...
  echo -e "\nOverall Descheduler - Validate EvictPodsWithLocalStorage & default for EvictPodsWithPVC profiles Testcase result:  FAIL"
