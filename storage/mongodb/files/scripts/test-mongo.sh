...
for load  in $(echo ${WORKLOAD} | sed -e s/,/" "/g); do
	for thread in $(echo ${THREADS} | sed -e s/,/" "/g); do
		echo "Threads-${thread}" > ${output_dir}/result_${NAMESPACE}_${load}_threads_${thread}.txt
		grep Throughput ${output_dir}/mongodb_run_load_${NAMESPACE}_${load}_threads_${thread}.txt | cut -d',' -f3 | cut -d' ' -f2 >> ${output_dir}/result_${NAMESPACE}_${load}_threads_${thread}.txt
	done
	paste -d',' ${output_dir}/result_${NAMESPACE}_${load}_threads_* > ${output_dir}/result_${NAMESPACE}_${load}_recordcount_${RECORDCOUNT}_operationcount_${OPERATIONCOUNT}.csv
done 
