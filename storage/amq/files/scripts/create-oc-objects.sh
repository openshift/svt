...
do
  ...
  NAMESPACE="${NAMESPACE_BASENAME}-${i}"
  MY_TIME=-1
  wait_until_the_pod_is_ready ${NAMESPACE} broker-amq 180 10
  if (( ${MY_TIME} == -1 )); then
    echo "broker-amq pod is not ready, time is up"
    exit 1
  else
    echo "it took ${MY_TIME} seconds to get broker-amq pod ready"
  fi
  MY_TIME=-1
  wait_until_the_pod_is_ready ${NAMESPACE} amq-perf 180 10
  if (( ${MY_TIME} == -1 )); then
    echo "amq-perf pod is not ready, time is up"
    exit 1
  else
    echo "it took ${MY_TIME} seconds to get amq-perf pod ready"
  fi
done
