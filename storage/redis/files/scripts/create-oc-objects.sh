...
readonly REDIS_PASSWORD=${6}
readonly VOLUME_CAPACITY=${7}
readonly REDIS_VERSION=${8}
readonly STORAGE_CLASS_NAME=${9}
...
function wait_until_the_pod_is_ready {
  ...
  while (( ($(date +%s) - ${start_time}) < ${timeout} ));
  do
    ready_pods=$(oc get pod -n ${project} | grep ${pod} | grep -v deploy | grep 1/1 | grep Running | wc -l)
...
do
  ...
  oc process -f "${TMP_FOLDER}/files/oc/redis-persistent-template.yaml" \
      -p MEMORY_LIMIT=${MEMORY_LIMIT} \
      -p REDIS_PASSWORD=${REDIS_PASSWORD} \
      -p VOLUME_CAPACITY=${VOLUME_CAPACITY} \
      -p REDIS_VERSION=${REDIS_VERSION} \
...
do
  ...
  NAMESPACE="${NAMESPACE_BASENAME}-${i}"
  MY_TIME=-1
  wait_until_the_pod_is_ready ${NAMESPACE} redis 180 10
  if (( ${MY_TIME} == -1 )); then
    echo "pod is not ready, time is up"
    exit 1
  else
    echo "it took ${MY_TIME} seconds to get redis pod ready"
  fi
done
