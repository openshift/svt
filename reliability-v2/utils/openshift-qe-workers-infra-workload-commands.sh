...
function create_machineset() {
	...
	*) echo "Invalid parameter, unsupported option ${FLAG}"
    ...
    echo "###########################################################################################"
    INFRA_TAINT=${INFRA_TAINT:="false"}
    if [[ $INFRA_TAINT == "true" ]];then
      mv /tmp/machineset.json /tmp/machineset_tmp.json
      jq '.spec.template.spec += {"taints":[{"key": "node-role.kubernetes.io/infra","value": "reserved","effect": "NoSchedule"},{"key": "node-role.kubernetes.io/infra","value": "reserved","effect": "NoExecute"}]}' /tmp/machineset_tmp.json > /tmp/machineset.json
      rm /tmp/machineset_tmp.json
    fi
    if [[ $MACHINESET_TYPE == "infra" ]];then
