...
oc patch -n openshift-ingress-operator ingresscontrollers.operator.openshift.io default -p '{"spec": {"nodePlacement": {"tolerations":[{"key": "node-role.kubernetes.io/infra","effect": "NoSchedule","value": "reserved"},{"key": "node-role.kubernetes.io/infra","effect": "NoExecute","value": "reserved"}]}}}' --type merge
...
max_retries=30
retry_times=1
while [[ $(oc get pods --no-headers -n openshift-monitoring | egrep -v "Completed|Running" | wc -l | tr -d ' ') != "0" ]];
do
    echo -n "." && sleep 5; 
    if [[ $retry_times -eq $max_retries ]];then
