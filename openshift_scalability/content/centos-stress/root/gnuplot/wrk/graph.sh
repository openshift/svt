...
graph_time_bytes_hits_latency() {
  ...
  mkdir -p $dir_out
  printf "# time	bytes_in	hits	latency\n" > $graph_data_in
  awk '
{
  ...
  if(time_prev != time) {
    printf "%ld\t%ld\t%ld\t%lf\n", time_prev, bytes_in, hits, (latency/hits)
    time_prev=time
    bytes_in=0
    hits=0
    latency=0
...
graph_time_bytes_per_endpoint() {
  ...
  awk -F, '{a[$5]}END{for (v in a) {print v}}' $results | LC_ALL=C sort > $unique_endpoints
...
main($5)				# processing of the second file
